...
//cout<< "Entry no." << "\t"<<jentry<< "\t" <<"No. of Gen photons"<<"\t"<< A_lead_Pho_Gen_Pt->size()+A_sublead_Pho_Gen_Pt->size()<<endl;
		...
		float recoDRNEnergy_ = recoDRNEnergy->at(0);
		float matchedGenEnergy = matchedGenEnergy_->at(0);
		float energy_ecal_mustache_ = energy_ecal_mustache->at(0);
	...
	vector<vector<float>*> ES_Pho_hit_phi = {Hit_ES_Phi_Pho1,Hit_ES_Phi_Pho2,Hit_ES_Phi_Pho3,Hit_ES_Phi_Pho4};
        ...
        Gen_H_Mass->Fill(Gen_H_mass);        Gen_H_Pt->Fill(Gen_H_pt);        Gen_H_Eta->Fill(Gen_H_eta);        Gen_H_Phi ->Fill(Gen_H_phi);        Gen_H_E ->Fill(Gen_H_En);        Gen_H_LB->Fill(Gen_H_lb); Gen_H_rapidity->Fill(Gen_h_rapidity);
